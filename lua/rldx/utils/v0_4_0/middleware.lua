...
function load.from_hex(value, opts, abort)
	...
	if value.organization ~= nil and value.properties ~= "" then
		value.organization = (value.organization:gsub("..", decode_hex_apply))
	else
		value.organization = ""
	end
	return value, opts, abort
...
function load.decrypt(value, opts, abort)
	...
	if value.organization ~= nil then
		value.organization = xor(value.organization, opts.key)
	else
		value.organization = ""
	end
...
function save.to_hex(entry, opts, abort)
	...
	if entry.organization ~= nil then
		entry.organization = (entry.organization:gsub(".", encode_hex_apply))
	else
		entry.organization = ""
	end
...
function save.encrypt(entry, opts, abort)
	...
	if entry.organization ~= nil then
		entry.organization = xor(entry.organization, opts.key)
	else
		entry.organization = nil
	end
...
function M.from_completions(data, opts)
	...
	for _, entry in ipairs(data) do
		...
		local org, firstname, lastname = string.match(entry.label, "%[(.+)%]([%w_]+)%.([%w_]+)")

		if (firstname ~= nil) and (lastname ~= nil) and (org ~= nil) then
			entry.label = firstname .. "." .. lastname
		...
		if entry.organization ~= nil then
			contact.organization = entry.organization
		end
